...
This is the bit of Method 4 that makes the throughput case work on **single-phase hybrid ESAs** (model number ending in EHA-G20). The inverter combines grid AC and solar DC together to charge at up to 13.5kW on the 10kW model, which HA can't ask for. The 13.5kW number is from GoodWe's own [ESA Series single-phase datasheet, V2.1 June 2026](https://www.goodwe.com.au/Ftp/EN/Downloads/Datasheet/GW_ESA-3-10kW_Datasheet-AU.pdf) - "Max. Charging Power" row for the GW9.999K-EHA-G20. Smaller single-phase models have proportionally lower ceilings (5kW model = 7.5kW, 8kW model = 12kW). Community confirmation: Whirlpool thread "Goodwe ESA maximum charge rate?" by user nutttr with confirmation from Zerosignal (<https://forums.whirlpool.net.au/thread/9kppp8k2>).

**Three-phase ESAs (ETA-G20) don't get the throughput boost, and neither do the AC-coupled ranges (BHA/BTA-G20) on either phase.** Per the [three-phase datasheet, V2.1 June 2026](https://www.goodwe.com.au/Ftp/EN/Downloads/Datasheet/GW_ESA-5-30kW_Datasheet-AU.pdf), max charging power equals nominal AC across the whole three-phase range; the AC-coupled units have no PV input to blend at all. You should still set up the TOU schedules below for the other Method 4 benefits (precise grid-export control, your TOU survives, the HA smart-layer at peak), just don't expect a 35% charge-rate bump. See the [strategy guide's model compatibility section](../automations/globird/README.md#model-and-firmware-compatibility) for the full model tables.
...
If you can't find it: the menu may be under "Advanced" or require an installer password (see the gotcha above about installer passwords differing between SolarGo and SEMS+). If TOU isn't there in SEMS+, fall back to SolarGo - some app version combinations still surface TOU there. The official [ESA user manual](https://admin.goodwe.com/Api/downloadFile?id=3105&mid=60&type=2) (July 2026) has a working-modes chapter with app screenshots if you want GoodWe's own walkthrough, and the Whirlpool threads ["Home Assistant setup with GoodWe inverter"](https://forums.whirlpool.net.au/thread/9xv6wp84) and ["GoodWe ESA - Setting export TOU with SOC limit"](https://forums.whirlpool.net.au/thread/9n111qlk) are the best places to ask if you're stuck.
