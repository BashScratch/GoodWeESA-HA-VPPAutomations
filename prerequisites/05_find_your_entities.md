...
- **You're running both the native and experimental integrations.** This is the most common cause of `_2` suffixes. When both are installed (which is the supported setup for Methods 2-4), they expose overlapping entities and HA appends `_2` to whichever instance registered second. From a real install we audited, this typically looks like: `sensor.goodwe_total_energy_export` without `_2`, but `sensor.goodwe_active_power_2`, `number.goodwe_grid_export_limit_2`, `button.goodwe_synchronize_inverter_clock_2`, and most of the `on_grid_l1_*` voltage/current/power/frequency sensors all with `_2`. The split is unpredictable and depends on the order you installed the integrations. The good news: both copies typically work; you just need to use whichever one has the live value when you set up the YAML.
- **You re-added the integration.** If you've ever installed and removed the GoodWe integration (or had it crash and re-set itself up), HA holds onto the original entity IDs for the deleted instance and appends `_2` to the fresh ones. Repeated install/remove cycles can produce `_3`, `_4` and so on. The fix is to remove the dead instance properly via Settings > Devices & services and rename the live entity (cog icon > Entity ID) to drop the suffix - or just live with the suffix and use whichever entity ID is actually receiving data.
