...
**If you're on a three-phase ESA (the middle letter of the suffix is T - e.g. GW9.999K-ETA-G20 instead of GW9.999K-EHA-G20), the throughput advantage doesn't apply to you.** Three-phase ESAs cap max charging at nominal AC across the whole range, with no AC+DC blending headroom. Method 3 is still worth using on three-phase for the other reasons (precise grid-export control, your TOU schedule survives, all the HA smart-layer benefits), just not for the charge-rate boost. See the [strategy guide's three-phase table](../#three-phase-esa---charge-rates-by-model) for the per-model numbers.
