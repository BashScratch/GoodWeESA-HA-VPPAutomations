...
1. In the GoodWe SEMS+ app, you create two TOU schedules: a charge slot (11:00-13:55, target 100% SOC) for the free window, and a discharge slot (peak window, 100% inverter power) for HA to manage on top. The firmware runs both. (Why 13:55 and not 14:00? The inverter's ~30-second wind-down after the slot ends would otherwise bill at shoulder rates - see [prereq 08](../../../prerequisites/08_sems_tou_schedule.md).)
...
- **Time:** 11:00 to 13:55 (ends 5 minutes early so the inverter's wind-down stays inside the free window - see prereq 08)
- **Mode:** Charge
- **SOC target:** 100%
...
- **Export limit master switch.** `switch.goodwe_grid_export_limit_switch` is the master enable for the export-limit feature - the inverter only enforces the value in `number.goodwe_grid_export_limit` when this switch is on. The YAML turns it on automatically at 17:56 and verifies the state before arming peak export; if for any reason it can't be enabled, the automation aborts with a notification rather than silently letting the inverter push at full power. You shouldn't need to touch it manually, but it's worth knowing it exists if a "Zero Hero Aborted - Export Limit Switch Off" notification lands on your phone.
- **App schedule layout.** Two TOU slots is correct for Method 4: a charge slot (11:00-13:55) and a discharge slot (18:00-21:00 at 100% inverter output). Don't add other slots in the app - they'll fight HA's logic at peak.
- **`select.goodwe_inverter_operation_mode` may read "general" during peak even though your TOU discharge is running.** This is expected on at least some firmware/integration combinations - the TOU schedule runs underneath the headline operation mode and the entity doesn't flip to `eco` to reflect it. Don't use that entity to judge whether your discharge slot is working; check that battery SOC is dropping and grid export is near your limit instead. (Method 4 deliberately never writes this entity, so it sitting at `general` is also confirmation HA hasn't clobbered your TOU schedule.)
