...
For example, a single-phase 10kW hybrid ESA can charge at up to 13.5kW by blending AC and DC power simultaneously (per GoodWe's own [ESA Series single-phase datasheet, V2.1 June 2026](https://www.goodwe.com.au/Ftp/EN/Downloads/Datasheet/GW_ESA-3-10kW_Datasheet-AU.pdf), "Max. Charging Power" row for the GW9.999K-EHA-G20). Some automation approaches will silently cap this at 10kW because they only command the AC side. Note that this throughput advantage applies only to single-phase **hybrid** models: three-phase ESAs (ETA-G20) are limited to their nominal AC rating, and the newer AC-coupled ranges (BHA/BTA-G20) have no PV input to blend at all - see the strategy guide's [model compatibility section](./automations/globird/README.md#model-and-firmware-compatibility).
